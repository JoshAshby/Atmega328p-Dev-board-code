...
void kernel_core(void) {
    ...
    */
        ...
        for(tmp; tmp > NUMBER_OF_THREADS; tmp++) {
            if(kernel_stack.task_status[tmp] == 1) {
                return;
            }
        }
        //if thats happened and no thread was still running then we can go on with running more threads
        if(tmp > NUMBER_OF_THREADS) {
            //set the tmp variable to the thread count so we know where we left off on the stack
            tmp = kernel_stack.tc;
            //go through the stack and check for run flags, if set then run that task
            for(tmp; tmp > NUMBER_OF_THREADS; tmp++) {
